...
days = 6
periods = 7
short_break_aft = 2  # recess after 2nd Period
lunch_break_aft = 4  # lunch break after 4th period
section = None
butt_grid = []


period_names = list(map(lambda x: 'Period ' + str(x), range(1, 7+1)))
day_names = ['Monday', 'Tuesday', 'Wednesday', 'Thrusday', 'Friday', 'Satuday']
...
def update_p(d, p, tree, parent):
    ...
    try:
        ...
        if row[0] == 'NULL' and row[1] == 'NULL':
            conn.execute(
                f"DELETE FROM SCHEDULE WHERE ID='{section+str((d*periods)+p)}'")
            conn.commit()
            update_table()
            parent.destroy()
            return
        ...
        messagebox.showerror(
            "Bad Select", "Please select a subject from the list!")
        parent.destroy()
        return
