...
days = 6
periods = 7
short_break_aft = 2  # short after 2nd Period
lunch_break_aft = 4  # lunch after 4th period
section = None
butt_grid = []
...
def update_table(sec):
    for i in range(days):
        for j in range(periods):
            ...
            butt_grid[i][j]['bg'] = 'black'
            if len(cursor) != 0:
                subcode = cursor[0][0]
                cur1 = conn.execute(
                    F"SELECT SUBTYPE FROM SUBJECTS WHERE SUBCODE='{subcode}'")
                cur1 = list(cur1)
                subtype = cur1[0][0]
...
def process_button(d, p, sec):
    ...
    tk.Label(details, text='Class Details', font=(
        'Consolas', 15, 'bold')).pack(pady=15)
    tk.Label(details, text='Day: ' +
             day_names[d], font=('Consolas'), anchor="w").pack(expand=1, fill=tk.X, padx=20)
    tk.Label(details, text='Period: '+str(p+1), font=('Consolas'),
             anchor="w").pack(expand=1, fill=tk.X, padx=20)
    tk.Label(details, text='Subject Code: '+subcode, font=('Consolas'),
             anchor="w").pack(expand=1, fill=tk.X, padx=20)
    tk.Label(details, text='Subect Name: '+subname, font=('Consolas'),
             anchor="w").pack(expand=1, fill=tk.X, padx=20)
    tk.Label(details, text='Subject Type: '+subtype, font=('Consolas'),
             anchor="w").pack(expand=1, fill=tk.X, padx=20)
    tk.Label(details, text='Faculty Initials: '+fini, font=('Consolas'),
             anchor="w").pack(expand=1, fill=tk.X, padx=20)
    tk.Label(details, text='Faculty Name: '+fname, font=('Consolas'),
             anchor="w").pack(expand=1, fill=tk.X, padx=20)
    tk.Label(details, text='Faculty Email: '+femail, font=('Consolas'),
             anchor="w").pack(expand=1, fill=tk.X, padx=20)
...
def student_tt_frame(tt, sec):
    ...
    for i in range(days):
        b = []
        for j in range(periods):
            if j < short_break_aft:
                bb = tk.Button(first_half)
                bb.grid(row=i+1, column=j+2)
            elif j < lunch_break_aft:
                bb = tk.Button(second_half)
                bb.grid(row=i+1, column=j+1)
            else:
                bb = tk.Button(third_half)
